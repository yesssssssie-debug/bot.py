...
# ==================== إعدادات GitHub (مباشرة في الكود) ====================
...
# ==================== إدارة العمليات (المعدلة) ====================
running_processes = {}
process_lock = threading.Lock()

def start_bot_process(bot_id: str, bot_file: str) -> bool:
    """تشغيل بوت كعملية منفصلة باستخدام os.system (يعمل على Railway)"""
    try:
        if not os.path.exists(bot_file):
            print(f"❌ ملف البوت غير موجود: {bot_file}")
            return False
        
        # تشغيل البوت في الخلفية باستخدام os.system
...
def stop_bot_process(bot_id: str) -> bool:
    ...
    try:
        ...
        print(f"خطأ في إيقاف البوت {bot_id}: {e}")
        return False

def monitor_bots():
    """مراقبة البوتات (مبسطة)"""
    while True:
        try:
            # لا يمكن مراقبة العمليات مع os.system
            # نتركها تعمل في الخلفية
            time.sleep(30)
        except Exception as e:
    ...
    try:
        ...
        if start_bot_process(bot_id, bot_file_path):
            ...
            bot.edit_message_text(
                ...
                f"📊 الحالة: 🟢 شغال\n"
                f"📅 الأيام المتبقية: {get_user_bot_days(user_id)}\n"
                f"📁 تم رفعه إلى GitHub: bots/{bot_name}/{bot_id}/",
                status_msg.chat.id,
