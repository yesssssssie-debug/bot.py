...
def process_bot_file(message):
    ...
    try:
        ...
        bot_file_path = os.path.join(BOTS_PATH, f"{bot_id}.py")
        
        with open(bot_file_path, "wb") as f:
            f.write(downloaded_file)
        
        bot_info = {"id": bot_id, "name": bot_name, "file": f"{bot_id}.py", "status": "waiting", "created": datetime.now().isoformat(), "user_id": user_id, "username": message.from_user.username or "لا يوجد"}
        ...
        bot.register_next_step_handler(msg, process_requirements_file, bot_id)
    except Exception as e:
        bot.reply_to(message, f"❌ خطأ: {str(e)}", reply_markup=back_button())

def process_requirements_file(message, bot_id):
    user_id = message.from_user.id
    ...
    try:
        ...
        req_file_path = os.path.join(BOTS_PATH, f"{bot_id}_requirements.txt")
