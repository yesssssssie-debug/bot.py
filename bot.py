...
TOKEN = "8350116285:AAF8snKoKOxD4p2KPVknlVpY91Cd5wI6eNI"
...
def init_database():
    ...
    try:
        ...
        pass
    try:
        cursor.execute("ALTER TABLE permanent_links ADD COLUMN views_count INTEGER DEFAULT 0")
    except:
        pass
...
def save_permanent_link(unique_id, file_path, file_type, enemy_name, admin_id, is_video=0, file_data=None, source_chat_id=None, source_message_id=None):
    try:
        ...
        cursor.execute('''INSERT INTO permanent_links 
                         (unique_id, file_path, file_type, enemy_name, admin_id, created_at, is_video, is_active, file_data, source_chat_id, source_message_id, views_count)
                         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
                      (unique_id, file_path, file_type, enemy_name, admin_id, datetime.now(), is_video, 1, file_data, source_chat_id, source_message_id, 0))
        conn.commit()
        conn.close()
        return unique_id
...
def increment_views(unique_id):
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("UPDATE permanent_links SET views = views + 1, views_count = views_count + 1 WHERE unique_id = ?", (unique_id,))
        conn.commit()
        conn.close()
    except:
        pass
...
def share_link_with_user(link_id, shared_by, shared_to):
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute('''INSERT INTO shared_links (link_id, shared_by, shared_to, shared_at)
                         VALUES (?, ?, ?, ?)''',
                      (link_id, shared_by, shared_to, datetime.now()))
        conn.commit()
        conn.close()
        return True
    except:
...
def get_shared_links(user_id):
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute('''SELECT pl.*, sl.shared_at, sl.shared_by 
                         FROM shared_links sl 
                         JOIN permanent_links pl ON sl.link_id = pl.id 
                         WHERE sl.shared_to = ? AND pl.is_active = 1 
                         ORDER BY sl.shared_at DESC''', (user_id,))
        results = cursor.fetchall()
        conn.close()
        return results
    except:
...
def get_main_keyboard(user_id):
    ...
    btn_links = InlineKeyboardButton("🔗 روابطي", callback_data="my_links")
    btn_search = InlineKeyboardButton("🔍 بحث بالاسم", callback_data="search_links")
    btn_shared = InlineKeyboardButton("📤 روابط مشتركة", callback_data="shared_links")
    markup.add(btn_channel, btn_bot_channel)
    markup.add(btn_admins, btn_support)
    markup.add(btn_links, btn_search)
    markup.add(btn_shared)
...
def backup_all_data():
    ...
    tables = ['permanent_links', 'admins', 'users', 'admin_channels', 
              'support_tickets', 'security_logs', 'broadcast_queue', 'deleted_links', 'shared_links']
    data = {}
    for table in tables:
...
def handle_callbacks(call):
    ...
    if call.data == "back_to_main":
    ...
    elif call.data == "search_links":
        bot.answer_callback_query(call.id)
        msg = bot.send_message(chat_id, "🔍 **أدخل اسم الخصم للبحث:**")
        bot.register_next_step_handler(msg, process_search_links)
    
    elif call.data == "shared_links":
        bot.answer_callback_query(call.id)
        shared = get_shared_links(user_id)
        
        if shared:
            text = "📤 **الروابط المشتركة معك:**\n\n"
            for link in shared:
                text += f"🔗 {link['enemy_name']}\n"
                text += f"📅 {link['created_at'][:10]}\n"
                text += f"👁️ {link['views']} مشاهدة\n"
                text += f"📤 مشاركة من: {get_user_by_id(link['shared_by']) or 'مجهول'}\n\n"
        else:
            text = "📭 لا توجد روابط مشتركة معك."
        ...
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT id, unique_id, enemy_name, created_at, views FROM permanent_links WHERE admin_id = ? AND is_active = 1 ORDER BY created_at DESC LIMIT 20", (user_id,))
        links = cursor.fetchall()
        conn.close()
        ...
        if links:
            text = "🔗 **روابطي:**\n\n"
            for link in links:
                text += f"• {link['enemy_name']} - `{link['unique_id']}`\n"
                text += f"  👁️ {link['views']} مشاهدة | 📅 {link['created_at'][:10]}\n\n"
            
            markup = InlineKeyboardMarkup(row_width=1)
            for link in links[:5]:
                markup.add(InlineKeyboardButton(f"📤 مشاركة {link['enemy_name']}", callback_data=f"share_{link['id']}"))
            markup.add(InlineKeyboardButton("🔙 القائمة الرئيسية", callback_data="back_to_main"))
        else:
            text = "📭 ليس لديك روابط."
            markup = InlineKeyboardMarkup()
        ...
        if user_id not in ADMINS:
            bot.answer_callback_query(call.id, "⛔ فقط الأدمن يمكنه المشاركة!", show_alert=True)
            return
        bot.answer_callback_query(call.id)
        
        link_id = int(call.data.split("_")[1])
        msg = bot.send_message(chat_id, "📤 **أدخل معرف المستخدم لمشاركة الرابط معه:**\nمثال: 123456789")
        bot.register_next_step_handler(msg, lambda m: process_share_link(m, link_id))
        ...
        admins_text = (
            ...
            f"• المالك الأساسي: [كايو](tg://user?id={OWNER_ID})\n"
            f"• أدمن النظام: [rest sword](tg://user?id={ADMIN_ID_1})\n"
            f"• أدمن النظام: [ابو نجم • SWORD](tg://user?id={ADMIN_ID_2})\n\n"
        ...
        shared_links = cursor.fetchone()[0] or 0
        ...
        text += f"📤 الروابط المشتركة: {shared_links}\n"
...
def process_search_links(message):
    chat_id = message.chat.id
    search_name = message.text.strip()
    
    if not search_name:
        bot.send_message(chat_id, "⚠️ اسم غير صالح.")
        return
    
    results = search_links_by_name(search_name)
    
    if results:
        text = f"🔍 **نتائج البحث عن '{search_name}':**\n\n"
        for link in results[:20]:
            text += f"• {link['enemy_name']} - `{link['unique_id']}`\n"
            text += f"  👁️ {link['views']} مشاهدة | 📅 {link['created_at'][:10]}\n\n"
        if len(results) > 20:
            text += f"...و {len(results) - 20} نتيجة أخرى"
    else:
        text = f"🔍 لا توجد نتائج للبحث عن '{search_name}'"
    
    markup = InlineKeyboardMarkup()
    markup.add(InlineKeyboardButton("🔙 القائمة الرئيسية", callback_data="back_to_main"))
    bot.send_message(chat_id, text, reply_markup=markup, parse_mode="Markdown")

def process_share_link(message, link_id):
    chat_id = message.chat.id
    user_id = message.from_user.id
    ...
    try:
        ...
        if target_user_id == user_id:
            bot.send_message(chat_id, "⚠️ لا يمكنك مشاركة الرابط مع نفسك!")
            return
        
        if is_user_blocked(target_user_id):
            bot.send_message(chat_id, "⚠️ المستخدم محظور!")
            return
        
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT unique_id, enemy_name FROM permanent_links WHERE id = ? AND is_active = 1", (link_id,))
        link = cursor.fetchone()
        conn.close()
        
        if not link:
            bot.send_message(chat_id, "⚠️ الرابط غير موجود!")
            return
        
        success = share_link_with_user(link_id, user_id, target_user_id)
        
        if success:
            bot_username = bot.get_me().username
            share_link = f"https://t.me/{bot_username}?start={link['unique_id']}"
            
            try:
                bot.send_message(
                    target_user_id,
                    f"📤 **تم مشاركة رابط معك!**\n\n"
                    f"🎯 الاسم: {link['enemy_name']}\n"
                    f"🔗 الرابط: [اضغط هنا]({share_link})\n"
                    f"👤 مشاركة من: {get_user_by_id(user_id) or 'مجهول'}"
                )
                bot.send_message(chat_id, f"✅ تم مشاركة الرابط بنجاح مع المستخدم `{target_user_id}`", parse_mode="Markdown")
            except:
                bot.send_message(chat_id, f"⚠️ تعذر إرسال الرابط للمستخدم. قد يكون قام بحظر البوت.")
        else:
            bot.send_message(chat_id, "❌ فشل مشاركة الرابط!")
            
    except ValueError:
        bot.send_message(chat_id, "⚠️ يرجى إدخال معرف مستخدم صحيح (أرقام فقط).")
...
def main():
    ...
    print("🔑 التوكن الجديد: 8350116285:AAF8snKoKOxD4p2KPVknlVpY91Cd5wI6eNI")
    ...
    print("📤 نظام مشاركة الروابط")
    print("👑 المالك: كايو")
    print("👤 الأدمن: rest sword, ابو نجم • SWORD")
