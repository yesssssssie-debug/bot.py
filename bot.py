...
class BotStates(StatesGroup):
    ...
    waiting_for_edit_admin_list = State()
...
def init_database():
    ...
    try:
        ...
        pass
    try:
        cursor.execute("ALTER TABLE admins ADD COLUMN admin_name TEXT")
    except:
        pass
    ...
    if user_id == OWNER_ID:
        markup.inline_keyboard.extend([
            ...
            [
                ...
                InlineKeyboardButton(text="🔙 العودة", callback_data="back_to_main", style="danger")
            ],
            [
...
async def handle_direct_photo(message: Message, state: FSMContext):
    user_id = message.from_user.id
    chat_id = message.chat.id
    ...
    try:
        ...
        await state.update_data(unique_id=unique_id, photo_path=photo_path, file_data=file_data, is_video=0)
...
async def handle_video(message: Message, state: FSMContext):
    ...
    try:
        ...
        await state.set_state(BotStates.waiting_for_enemy_name)
        await state.update_data(unique_id=unique_id, video_path=video_path, file_data=file_data, is_video=1)
...
async def handle_callbacks(call: CallbackQuery, state: FSMContext):
    ...
    # ========== CHANGE WELCOME MESSAGE ==========
        ...
        await bot.send_message(
            ...
            "- `{RIGHTS_CHANNEL}` لقناة المطور\n"
            "- `{BOT_CHANNEL}` لقناة البوت\n\n"
            "مثال:\n"
            "`مرحباً بك في بوت ازالات السورد\n👤 المستخدمين: {users_count}`"
        )
    ...
    # ========== EDIT OWNER NAME ==========
    elif call.data == "edit_owner_name":
        if user_id != OWNER_ID:
        ...
        await state.update_data(edit_user_id=OWNER_ID)
        await bot.send_message(chat_id, "✏️ أرسل الاسم الجديد للمالك:")
    ...
    # ========== EDIT ADMIN 1 NAME ==========
        ...
        await state.set_state(BotStates.waiting_for_edit_admin_name)
        await state.update_data(edit_user_id=ADMIN_ID_1)
        await bot.send_message(chat_id, "✏️ أرسل الاسم الجديد لأدمن 1:")
    
    # ========== EDIT ADMIN 2 NAME ==========
    elif call.data == "edit_admin2_name":
        if user_id != OWNER_ID:
            await bot.answer_callback_query(call.id, "⛔ غير مصرح!", show_alert=True)
            return
        
        await state.set_state(BotStates.waiting_for_edit_admin_name)
        await state.update_data(edit_user_id=ADMIN_ID_2)
        await bot.send_message(chat_id, "✏️ أرسل الاسم الجديد لأدمن 2:")
    
    # ========== EDIT EXTRA ADMIN NAME ==========
    elif call.data.startswith("edit_admin_"):
        if user_id != OWNER_ID:
            await bot.answer_callback_query(call.id, "⛔ غير مصرح!", show_alert=True)
            return
        
        admin_id = int(call.data.replace("edit_admin_", ""))
        await state.set_state(BotStates.waiting_for_edit_admin_name)
        await state.update_data(edit_user_id=admin_id)
        await bot.send_message(chat_id, f"✏️ أرسل الاسم الجديد للأدمن {admin_id}:")
...
async def process_welcome_message(message: Message, state: FSMContext):
    ...
    # استبدال المتغيرات
    users_count = get_users_count()
    welcome_text = welcome_text.replace("{users_count}", str(users_count))
    welcome_text = welcome_text.replace("{RIGHTS_CHANNEL}", RIGHTS_CHANNEL)
    welcome_text = welcome_text.replace("{BOT_CHANNEL}", BOT_CHANNEL)
